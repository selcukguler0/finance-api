...
import { initAppwrite } from "../../../utils/appwrite-connection";
...
export default async function handler(req, res) {
	...
	let database = new Appwrite.Databases(sdk,"631c28914d398195b491");
	// const databases = new database(client, '631c28914d398195b491');
	const promise = await database.createDocument(process.env.APPWRITE_COLLECTION_ID, uuidv4(), { "crypto": "btc" });
	console.log(promise);
	const loop = [];
	for (let i = 1; i <= 90; i++) {
		loop.push(i);
	...
	// 	console.log(i);
	// 	i = (i * 100)-100;
	// 	const data = await getAllCryptos(i);
	// 	for(const element of data){
	// 		await writeDatabase(element);
	// 	}
	// }
	// * 82 seconds
	// await Promise.all(loop.map(async (i) => {
	// 	const data = await getAllCryptos(i);
...
async function writeDatabase(element) {

}
// async function writeDatabase(element) {
// 	connection.query(
// 		"SELECT name FROM allCryptos WHERE name = ?", [element.name],
// 		(error,
// 			results) => {
// 			if (results.length === 0) {
// 				connection.query(
// 					"INSERT INTO allCryptos(name, shortName, symbol, price, `change`, changePercent, marketCap, Volume24Hr, totalVolume, circulatingSupply) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [element.name, element.shortName, element.symbol, element.price, element.change, element.changePercent, element.marketCap, element["Volume24Hr"], element.totalVolume, element.circulatingSupply],
// 					(error,
// 						results) => {
...
// 			} else {
// 				connection.query(
// 					"UPDATE allCryptos SET name = ?, shortName = ?, symbol = ?, price = ?, `change` = ?, changePercent = ?, marketCap = ?, Volume24Hr = ?, totalVolume = ?, circulatingSupply = ? WHERE name = ?", [element.name, element.shortName, element.symbol, element.price, element.change, element.changePercent, element.marketCap, element["Volume24Hr"], element.totalVolume, element.circulatingSupply, element.name],
// 					(error,
// 						results) => {
// 						console.log(error || "success");
// 					}
...
// 	)
// }
async function getAllCryptos(page) {
	try {
		const url =
			`https://finance.yahoo.com/cryptocurrencies?offset=${page}&count=100`;
		...
		const xShortName = dom.window.document.querySelectorAll('[data-test="quoteLink"]')
		...
		for (var x in xName) {
			cryptos.push({
				...
				shortName: xShortName[i]?.textContent,
